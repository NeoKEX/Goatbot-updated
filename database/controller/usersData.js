        ...
        async function create_(userID, userInfo) {
                ...
                const queue = new Promise(async function (resolve_, reject_) {
                        try {
                                ...
                                if (!userInfo) {
                                        log.warn("CREATE_USER", `Unable to fetch user info for userID: ${userID}. User may have privacy restrictions or invalid ID.`);
                                        throw new CustomError({
                                                name: "USER_INFO_NOT_AVAILABLE",
                                                message: `Cannot retrieve user information for userID "${userID}". The user may have privacy restrictions, be deactivated, or the ID is invalid.`
                                        });
                                }
                ...
                return new Promise(async function (resolve, reject) {
                        taskQueue.push(async function () {
                                try {
                                        ...
                                        if (!updateInfoUser) {
                                                log.warn("REFRESH_USER_INFO", `Unable to fetch updated user info for userID: ${userID}. User may have privacy restrictions or invalid ID.`);
                                                throw new CustomError({
                                                        name: "USER_INFO_NOT_AVAILABLE",
                                                        message: `Cannot retrieve user information for userID "${userID}". The user may have privacy restrictions, be deactivated, or the ID is invalid.`
                                                });
